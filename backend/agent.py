...
CLEAN_TRANSCRIPT_PROMPT = (
    ...
    "语音识别出来的原始转写里夹杂着寒暄、语气词和重复。你的任务只有一个：把它清洗成规范的临床记录。\n\n"
    "【必须删除】\n"
    "1. 寒暄、客套、称呼、与本次诊疗无关的闲聊（如“你好”“坐吧”“外面下雨了”）\n"
    "2. 语气词与口头禅（如“嗯”“啊”“那个”“就是说”“然后呢”）\n"
    "3. 结巴、重复、自我更正、说了半句又重来的无意义片段\n\n"
    "【必须保留并提炼】\n"
    "1. 脉象、舌象、症状、主诉等关键描述，一个都不能漏\n"
    "2. 把口误和大白话改写成规范中医术语（如“舌头胖胖的边上还有牙齿印”→“舌体胖大有齿痕”）\n"
    "3. 只做去噪、提炼和措辞规范，不得改变原意\n\n"
    "【绝对禁止】\n"
    "1. 不能做诊断、辨证、开方或任何推理判断\n"
    "2. 不能虚构老师没说过的症状、舌象、脉象，也不许“顺手补全”\n\n"
    "【输出格式】严格按下面五项，每项一行，不增不减：\n"
    "主诉：...\n"
    "现病史：...\n"
    "伴随症状：...\n"
    "舌象：...\n"
    "脉象：...\n\n"
    "【缺失处理】某一项在老师原话里没有提到，就写（未提及），不要凭空填写。\n\n"
    "【输出要求】只输出上述结构化文本本身，不要任何解释、说明、前言、后缀或 markdown 标记。\n\n"
    "老师原话：{raw_text}"
)
